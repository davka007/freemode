...
namespace freemode
{
    class mysql
    {
        ...
        public static void InitConnection()
        {
            ...
            {
                _connection.Open();
                NAPI.Util.ConsoleOutput("Epitixhs syndesh sto server MYSQL");
            }
            catch (Exception ex)
            {
                NAPI.Util.ConsoleOutput("Anepitixis syndesy sto server MYSQL");
                NAPI.Util.ConsoleOutput("Ekseresi: " + ex);
                NAPI.Task.Run(() =>
                {
                    Environment.Exit(0);
        ...
        public static void NewAccountRegister(Accounts account, string password)
        {
            ...
            {
                NAPI.Util.ConsoleOutput("Ekseresh: " + ex.ToString());
            }
        }
        ...
        public static bool IsValidPassword(string name, string inputPw)
        {
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
            {
                ...
                {
                    reader.Read();
                    temppas = reader.GetString("pass");
                }
            }
            if (BCrypt.CheckPassword(inputPw, temppas)) return true;
            return false;
